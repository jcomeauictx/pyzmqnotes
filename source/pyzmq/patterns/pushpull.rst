...
.. topic:: Pipeline pattern
    ...
    This is equivalent to the producer/consumer model but the results computed by
    a consumer are not sent upstream but downstream to another pull/consumer socket.
...
Producers are created with ZMQ.PUSH socket types. Producer is bound to well known port to which consumers can connect.    
...
Consumers are created with ZMQ.PULL socket types to pull requests from producers and use a push socket to connect and push result to result collector.  
